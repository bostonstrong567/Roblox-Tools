...
function FPSBoost:modifyDescendant(descendant)
    local originalValues = self.originalValues[descendant] or {}
    ...
    if descendant:IsA("BasePart") and not descendant:IsA("Texture") then
        if self.settings.LowQualityParts then
            ...
            descendant.Material = originalValues.Material or descendant.Material
            descendant.Transparency = originalValues.Transparency or descendant.Transparency
        end
    end
    ...
    if descendant:IsA("ParticleEmitter") or descendant:IsA("Trail") then
        if self.settings.DisableParticles then
            ...
            descendant.Enabled = originalValues.Enabled or descendant.Enabled
        end
    end
    ...
    for _, e in ipairs(game.Lighting:GetChildren()) do
        if e:IsA("BlurEffect") or e:IsA("SunRaysEffect") or e:IsA("ColorCorrectionEffect") or e:IsA("BloomEffect") or e:IsA("DepthOfFieldEffect") then
            if self.settings.DisableEffects then
                ...
                e.Enabled = originalValues and originalValues.Enabled or true
            end
        end
    end
end
